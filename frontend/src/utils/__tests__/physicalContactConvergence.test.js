...
describe('FT-B-001-S4 — TEST S4-K/N : matrice API PhysicalContact du catalogue', () => {
  const CATALOGUE = [
    ...
    'PIR_MOTION_SENSOR', 'TILT_SENSOR', 'IR_RECEIVER', 'HC_SR04', 'INDUCTOR', 'ZENER_DIODE', 'H_BRIDGE', 'AND_GATE',
  ]

  it('le catalogue compte exactement 42 types', () => {
    ...
    expect(new Set(CATALOGUE).size).toBe(42)
  ...
  it('classification breadboardInsertable finale S5 : 29 enfichables / 8 non-directs', () => {
    const INSERTABLE = ['RESISTOR', 'LED', 'DIODE', 'CAPACITOR', 'LDR', 'THERMISTOR',
      ...
      'ZENER_DIODE', 'PNP_TRANSISTOR', 'NMOS', 'PMOS', 'RELAY', 'VOLTAGE_REGULATOR', 'H_BRIDGE', 'AND_GATE']
