...
describe('MB-BREADBOARD-003 (correctif ciblé) — placement breadboard au dépôt initial (ADD_COMPONENT)', () => {
  ...
  // [FT-B-001-S5] MB-BREADBOARD-008 direct placement SUPERSÉDÉ pour ARDUINO :
  // ce composant n'est PAS directement enfichable (ses contacts sont
  // `breadboardInsertable: false`, il se relie au breadboard PAR FIL).
  // ADD_COMPONENT avec un breadboard posé retombe donc sur le snap-to-grid
  // GRID_SIZE existant — computeBreadboardPlacement() renvoie compatible:false
  // et n'aligne pas le corps sur des trous.
  it('TEST 4 ([FT-B-001-S5], ARDUINO non enfichable) : avec breadboard + ARDUINO, ADD_COMPONENT retombe sur le snap-to-grid GRID_SIZE (aucun alignement breadboard du corps)', () => {
    const { result } = renderHook(() => ({ ...useCircuit(), ...useCircuitInteraction() }), { wrapper })
    ...
    // GRID_SIZE = 20 : snapToGrid(178) = 180, snapToGrid(201) = 200.
    expect(arduino.x).toBe(180)
    expect(arduino.y).toBe(200)
    expect(arduino.x % 20).toBe(0)
    expect(arduino.y % 20).toBe(0)
  })
