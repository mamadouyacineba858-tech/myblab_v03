/**
 * tmp36A7C1.test.jsx — Ticket A7-C1 (TMP36) — DOM rendering assertions only.
 *
 * The bulk of A7-C1's test coverage lives in the sibling `.js` file
 * `tmp36A7C1.test.js`, which needs neither JSX nor `render()`. This `.jsx`
 * file is intentionally small: it holds ONLY the assertions that require
 * actually rendering `<Tmp36Part />` (backend raster proof, <picture>/
 * <source>/<img> DOM shape, WebP-with-PNG-fallback) — same split convention
 * as lightBulbA6Out2.test.js/.jsx.
 */
...
describe('A7-C1 — renderer utilise réellement le backend raster', () => {
  it('getComponentPresentation("TMP36") === raster (bareBody + markerless dérivés)', () => {
  ...
  it('le DOM rendu contient un <img> réel (aucun <svg>), dimensionné sur la boîte canonique', () => {
    ...
    const { container } = render(<Tmp36Part />)
...
describe('A7-C1 — résolution d\'asset (défaut 1x + haute densité 3x), fallback WebP/PNG', () => {
  const ASSET_RE = /^\/assets\/components\/tmp36\/tmp36\.default\.(1x|3x)\.(webp|png)( \dx)?$/
  ...
  it('la variante haute densité 3x est référencée dans le srcSet (système de résolution par densité existant, <picture>/srcSet natif)', () => {
    ...
    expect(img.getAttribute('src')).toMatch(/tmp36\.default\.3x\.png$/)
  })

  it('fallback WebP/PNG : <picture><source type="image/webp"> avant <img src=".png">, même patron que LIGHT_BULB/VIBRATION_MOTOR/…', () => {
    const { container } = render(<Tmp36Part />)
