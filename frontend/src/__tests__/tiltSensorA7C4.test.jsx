/**
 * tiltSensorA7C4.test.jsx — Ticket A7-C4-TILT (Tilt Sensor) — DOM rendering
 * assertions only.
 *
 * The bulk of A7-C4-TILT's test coverage lives in the sibling `.js` file
 * `tiltSensorA7C4.test.js`, which needs neither JSX nor `render()`. This
 * `.jsx` file is intentionally small: it holds ONLY the assertions that
 * require actually rendering `<TiltSensorPart />` (backend raster proof,
 * <picture>/<source>/<img> DOM shape, WebP-with-PNG-fallback) — same split
 * convention as pirMotionSensorA7C4.test.js/.jsx.
 */
...
describe('A7-C4-TILT — renderer utilise réellement le backend raster', () => {
  it('getComponentPresentation("TILT_SENSOR") === raster (bareBody + markerless dérivés)', () => {
  ...
  it('le DOM rendu contient un <img> réel (aucun <svg>), dimensionné sur la boîte canonique', () => {
    ...
    const { container } = render(<TiltSensorPart />)
...
describe('A7-C4-TILT — résolution d\'asset (défaut 1x + haute densité 3x), fallback WebP/PNG', () => {
  const ASSET_RE = /^\/assets\/components\/tilt-sensor\/tilt-sensor\.default\.(1x|3x)\.(webp|png)( \dx)?$/
  ...
  it('la variante haute densité 3x est référencée dans le srcSet (système de résolution par densité existant, <picture>/srcSet natif)', () => {
    ...
    expect(img.getAttribute('src')).toMatch(/tilt-sensor\.default\.3x\.png$/)
  })

  it('fallback WebP/PNG : <picture><source type="image/webp"> avant <img src=".png">, même patron que PIR_MOTION_SENSOR/TMP36', () => {
    const { container } = render(<TiltSensorPart />)
