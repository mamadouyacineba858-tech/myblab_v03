...
describe('MB-BREADBOARD-002 — Breadboard.jsx (Presentation, AC-18/LOCK-08)', () => {
  ...
  // (occupiedBy via resolveComponentContactHoles()/holeAt(), même mécanisme
  // que RESISTOR ci-dessus), par aucune fixture dédiée.
  ...
  // pattes ; LED_VISUAL_PINS 0/80 supprimé en FT-B-001-S4). La position
  // x:-2,y:12 ci-dessous est exactement celle retournée par
  // computeBreadboardPlacement(breadboard,'LED',{x:1,y:15},[]) pour ce même
  // BREADBOARD en (0,0) -> {valid:true, position:{x:-2,y:12},
  // holes:[{pinId:'anode',column:2,row:6},{pinId:'cathode',column:4,row:6}]}
  // (script Node jetable, jamais calculée à la main), pour confirmer 2 trous
  // distincts occupés (anode -> col2/row6, cathode -> col4/row6).
  it("met en évidence les deux trous occupés par les pins d'une LED (écart 24 = 2·BREADBOARD_PITCH, algorithme de placement généralisé)", () => {
    const led1 = { uid: 'led1', type: 'LED', x: -2, y: 12 }
    const { container } = render(<Breadboard breadboard={BREADBOARD} components={[led1]} />)
    const occupied = container.querySelectorAll('.breadboard__hole--occupied')
    // Anode (col2/row6) et cathode (col4/row6) tombent chacune sur un trou
