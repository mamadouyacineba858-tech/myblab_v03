...
describe('G — Backend Contract', () => {
  ...
  it('MB-VIS-INDUSTRIAL-001 — la présentation est réellement branchée dans le RendererRegistry / VisualizationManager', () => {
    ...
    // [MB-L1-PROP-005] CAPACITOR : retour au backend raster réel (Candidate
    // C, capacitorMarking.js) — [MB-L1-CONS-002] avait temporairement retiré
    // 'raster' pour un renderer CSS/DOM ; THERMISTOR reste 'svg' (hors
    ...
    // [MB-L1-PROP-005] backend 'raster' (Candidate C), bareBody/markerless
    // déclarés explicitement à true dans defaultRegistrations.js — même
    // contrat que RESISTOR/DIODE/LED.
  ...
  it('MB-VIS — composants raster déclarés à ce jour : RESISTOR (001C) + DIODE (002) + LED (003) + CAPACITOR (004/MB-L1-PROP-005) + LDR (005) + DC_MOTOR (007) + BUTTON + BUTTON_LATCHING (008) + BUZZER (031) + POTENTIOMETER (032) + RGB_LED (033) + NPN_TRANSISTOR (034) + SERVO (035) + POWER (036) + ARDUINO (037)', () => {
    // [MB-L1-PROP-005] CAPACITOR revient au backend raster (Candidate C) —
    // [MB-L1-CONS-002] l'avait temporairement porté à un renderer CSS/DOM
    // (backend 'svg'). THERMISTOR (006) reste 'svg', hors périmètre de ce
    ...
    // A6-OUT3 : HOBBY_GEARMOTOR rejoint également rasterTypes (paquet
    // d'assets réaliste Founder-approved "VERTICAL FINAL", motoréducteur).
    // A7-C1 : TMP36 rejoint également rasterTypes (paquet d'assets réaliste
    // Founder-approved R3).
    ...
    // A7-C4-PIR : PIR_MOTION_SENSOR rejoint également rasterTypes (paquet
    // d'assets réaliste Founder-approved, HC-SR501-style).
    // A7-C4-TILT : TILT_SENSOR rejoint également rasterTypes (paquet
    // d'assets réaliste Founder-approved, SW-520D-style).
    // A7-C4-IR : IR_RECEIVER rejoint également rasterTypes (paquet
    ...
    // A9-NOR : NOR_GATE rejoint également rasterTypes (paquet d'assets
    // Founder PASS FROZEN, même pipeline que AND_GATE/OR_GATE/NAND_GATE).
    // A9-XOR : XOR_GATE rejoint également rasterTypes (paquet d'assets
    // Founder PASS FROZEN, même pipeline que AND_GATE/OR_GATE/NAND_GATE/NOR_GATE).
    // A9-NOT : NOT_GATE rejoint également rasterTypes (paquet d'assets
    // Founder PASS FROZEN, même pipeline que les cinq portes précédentes).
    // A9-JK1 : JK_FLIP_FLOP_74HC73 rejoint également rasterTypes (paquet d'assets
    // Founder PASS FROZEN / CSA LOCKED, DIP-14). A9-DFF1 : D_FLIP_FLOP_74HC74 également
    // (paquet Founder PASS FROZEN / CSA LOCKED, DIP-14). A9-LATCH1 : D_LATCH_74HC75
    // également (paquet Founder PASS FROZEN / CSA LOCKED, DIP-16). A9-COUNTER1 :
    // BINARY_COUNTER_74HC161 également (paquet Founder PASS FROZEN / CSA LOCKED, DIP-16).
    // A10-DISP1 : SEVEN_SEGMENT_DISPLAY également (paquet Founder PASS FROZEN / CSA LOCKED, SC56-11EWA).
    expect(rasterTypes.slice().sort()).toEqual(['AND_GATE', 'ARDUINO', 'BATTERY_9V', 'BATTERY_AA', 'BINARY_COUNTER_74HC161', 'BUTTON', 'BUTTON_LATCHING', 'BUZZER', 'CAPACITOR', 'COIN_CELL_CR2032', 'DC_MOTOR', 'DIODE', 'DIP_SWITCH', 'D_FLIP_FLOP_74HC74', 'D_LATCH_74HC75', 'FLEX_SENSOR', 'FORCE_SENSOR', 'HC_SR04', 'HOBBY_GEARMOTOR', 'H_BRIDGE', 'INDUCTOR', 'IR_RECEIVER', 'JK_FLIP_FLOP_74HC73', 'LDR', 'LED', 'LIGHT_BULB', 'NAND_GATE', 'NMOS', 'NOR_GATE', 'NOT_GATE', 'NPN_TRANSISTOR', 'OR_GATE', 'PIR_MOTION_SENSOR', 'PMOS', 'PNP_TRANSISTOR', 'POLARIZED_CAPACITOR', 'POTENTIOMETER', 'POWER', 'RELAY', 'RESISTOR', 'RGB_LED', 'SERVO', 'SEVEN_SEGMENT_DISPLAY', 'SLIDE_SWITCH', 'SOIL_MOISTURE_SENSOR', 'TILT_SENSOR', 'TMP36', 'VIBRATION_MOTOR', 'VOLTAGE_REGULATOR', 'XOR_GATE', 'ZENER_DIODE'])
