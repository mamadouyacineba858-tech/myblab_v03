...
describe('G — Backend Contract', () => {
  ...
  it('MB-VIS — composants raster déclarés à ce jour : RESISTOR (001C) + DIODE (002) + LED (003) + CAPACITOR (004/MB-L1-PROP-005) + LDR (005) + DC_MOTOR (007) + BUTTON + BUTTON_LATCHING (008) + BUZZER (031) + POTENTIOMETER (032) + RGB_LED (033) + NPN_TRANSISTOR (034) + SERVO (035) + POWER (036) + ARDUINO (037)', () => {
    ...
    expect(rasterTypes.slice().sort()).toEqual(['AND_GATE', 'ARDUINO', 'BATTERY_9V', 'BATTERY_AA', 'BUTTON', 'BUTTON_LATCHING', 'BUZZER', 'CAPACITOR', 'COIN_CELL_CR2032', 'DC_MOTOR', 'DIODE', 'DIP_SWITCH', 'FLEX_SENSOR', 'FORCE_SENSOR', 'HC_SR04', 'HOBBY_GEARMOTOR', 'H_BRIDGE', 'INDUCTOR', 'IR_RECEIVER', 'LDR', 'LED', 'LIGHT_BULB', 'NAND_GATE', 'NMOS', 'NOR_GATE', 'NOT_GATE', 'NPN_TRANSISTOR', 'OR_GATE', 'PIR_MOTION_SENSOR', 'PMOS', 'PNP_TRANSISTOR', 'POLARIZED_CAPACITOR', 'POTENTIOMETER', 'POWER', 'RELAY', 'RESISTOR', 'RGB_LED', 'SERVO', 'SLIDE_SWITCH', 'SOIL_MOISTURE_SENSOR', 'TILT_SENSOR', 'TMP36', 'VIBRATION_MOTOR', 'VOLTAGE_REGULATOR', 'XOR_GATE', 'ZENER_DIODE'])
