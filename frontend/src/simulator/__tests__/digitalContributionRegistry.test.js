...
describe("A7-C3-PREQ — T01 : Registry inconnu -> null/absent", () => {
  ...
  it("registre de production : SOIL_MOISTURE_SENSOR (A7-C3) + PIR_MOTION_SENSOR (A7-C4-PIR) + TILT_SENSOR (A7-C4-TILT) + IR_RECEIVER (A7-C4-IR) + AND_GATE (A9-AND) + OR_GATE (A9-OR) + NAND_GATE (A9-NAND) + NOR_GATE (A9-NOR) + XOR_GATE (A9-XOR) + NOT_GATE (A9-NOT), aucun autre type", () => {
    expect(getAllDigitalContributionTypes()).toEqual(["SOIL_MOISTURE_SENSOR", "PIR_MOTION_SENSOR", "TILT_SENSOR", "IR_RECEIVER", "AND_GATE", "OR_GATE", "NAND_GATE", "NOR_GATE", "XOR_GATE", "NOT_GATE"])
  })
})
