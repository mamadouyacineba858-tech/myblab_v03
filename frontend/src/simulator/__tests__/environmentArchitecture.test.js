...
describe("A7-C0 — le moteur générique environmentalStimulus.js ne connaît aucun stimulus kind par son nom (I-A7C0-08/I-A7C0-09/T28/T29)", () => {
  ...
  it("A7-C1/A7-C2/A7-C3/A7-C4-PIR : LDR/TMP36/FORCE_SENSOR/FLEX_SENSOR/SOIL_MOISTURE_SENSOR/PIR_MOTION_SENSOR sont les SEULS composants de production enregistrés dans environmentalResponseRegistry.js — aucun autre type n'y est ajouté (T27/T37)", () => {
    ...
    // MOTION) — cette assertion verrouille désormais qu'AUCUN AUTRE type de
    // production (A7-C5+) n'a été ajouté en même temps, jamais que ces six
    // types soient absents.
    const source = readSourceWithoutComments(envRegistryPath)
    const registeredTypes = [...source.matchAll(/^\s*([A-Z][A-Z0-9_]*):\s*Object\.freeze\(\{\s*stimulus:/gm)].map((m) => m[1])
    expect(registeredTypes.sort()).toEqual(["FLEX_SENSOR", "FORCE_SENSOR", "LDR", "PIR_MOTION_SENSOR", "SOIL_MOISTURE_SENSOR", "TMP36"])
  })
})
