 ...
 * V21 : renforcement de l'émission lumineuse en état ON, sans modifier la géométrie électrique.
 */
export function LedPart({ isOn, uid }) {
  const lensMain = isOn ? '#f5222d' : '#8e1f24'
  const lensDark = isOn ? '#9d0711' : '#4f1116'
  const lensLight = isOn ? '#ff8f93' : '#bd454b'
  ...
  return (
    <div
      ...
      style={{ width: '100%', height: '100%', background: 'transparent', border: 0, borderRadius: 0, boxShadow: 'none', overflow: 'visible', display: 'block', position: 'relative', filter: isOn ? 'drop-shadow(0 0 5px rgba(255, 45, 45, 0.9))' : 'none' }}
    >
      <svg viewBox="0 0 80 64" width="80" height="64" role="img" aria-hidden="true" overflow="visible" style={{ display: 'block', overflow: 'visible' }}>
        <defs>
          ...
          <radialGradient id={`${id}-dome`} cx="34%" cy="18%" r="84%">
            <stop offset="0%" stopColor="#fff0f0" stopOpacity={isOn ? 0.9 : 0.34} /><stop offset="25%" stopColor={lensLight} stopOpacity={isOn ? 0.86 : 0.72} /><stop offset="62%" stopColor={lensMain} stopOpacity="0.96" /><stop offset="100%" stopColor={lensDark} />
          </radialGradient>
          ...
          <radialGradient id={`${id}-glow`} cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#ffffff" stopOpacity="1" /><stop offset="14%" stopColor="#fffbd0" stopOpacity="1" /><stop offset="34%" stopColor="#fff36b" stopOpacity="0.98" /><stop offset="62%" stopColor="#ff4b45" stopOpacity="0.7" /><stop offset="100%" stopColor="#ff2525" stopOpacity="0" />
          </radialGradient>
        ...
        <path d="M17 36 V15.4 C17 7.9 26.6 3 40 3 C53.4 3 63 7.9 63 15.4 V36 Z" fill={`url(#${id}-dome)`} opacity={isOn ? 0.96 : 0.84} />
        <path d="M16 16 C16.8 7.8 26.8 2.5 40 2.2 C30.7 4 24.5 9.4 24.5 16 V36 H17 Z" fill="#ffd0d0" opacity={isOn ? 0.28 : 0.18} />
        ...
        <g clipPath={`url(#${id}-glass-clip)`}>
          {isOn && <circle cx="40" cy="31.2" r="19" fill={`url(#${id}-glow)`} opacity="0.98" />}
          ...
          {isOn && <circle cx="40" cy="31.2" r="5" fill="#fffef0" opacity="1" />}
        </g>

        <ellipse cx="30.5" cy="8.2" rx="7" ry="2.25" fill="#ffe6e6" opacity={isOn ? 0.98 : 0.66} transform="rotate(-18 30.5 8.2)" />
        <path d="M20 25 C21 15.8 26.2 8.5 33.5 5.7" fill="none" stroke="#fff0f0" strokeWidth="1.3" strokeLinecap="round" opacity={isOn ? 0.92 : 0.54} />
