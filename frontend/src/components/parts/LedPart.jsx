 ...
 * - cathode : (52,62)
 *
 * V11 : correction de silhouette uniquement. Le dôme est plus haut et
 * plus volumineux, tandis que les deux pattes restent exactement sur les
 * endpoints électriques validés.
 */
export function LedPart({ isOn, uid }) {
  ...
  return (
    <div className={`part-led ${isOn ? 'part-led--on' : ''}`} aria-label={isOn ? 'LED allumée' : 'LED éteinte'} style={{ width: '100%', height: '100%', background: 'transparent', border: 0, borderRadius: 0, boxShadow: 'none', overflow: 'visible', display: 'block', position: 'relative', filter: isOn ? 'drop-shadow(0 0 3px rgba(255, 55, 55, 0.55))' : 'none' }}>
      <svg viewBox="0 0 80 64" width="80" height="64" role="img" aria-hidden="true" overflow="visible" style={{ display: 'block', overflow: 'visible' }}>
        ...
        <path d="M52 48 V62" fill="none" stroke={`url(#${id}-metal)`} strokeWidth="3.2" strokeLinecap="round" />
        <path d="M27.35 49 V61.35" fill="none" stroke="#f7f9fb" strokeWidth="0.7" strokeLinecap="round" opacity="0.72" />
        <path d="M51.35 49 V61.35" fill="none" stroke="#f7f9fb" strokeWidth="0.7" strokeLinecap="round" opacity="0.72" />
