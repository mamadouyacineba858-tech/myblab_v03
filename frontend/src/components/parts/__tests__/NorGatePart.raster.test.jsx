...
import { NorGatePart } from '../NorGatePart.jsx'
...
// reference, same method as OrGatePart.raster.test.jsx/NandGatePart.raster.test.jsx.
...
describe('A9-NOR raster and frozen source', () => {
  it('registered raster renderer uses only the approved picture', () => {
    expect(DEFAULT_REGISTRATIONS.filter(r => r.type === 'NOR_GATE')).toEqual([
      { type: 'NOR_GATE', component: NorGatePart, visual: { backend: 'raster' } },
    ])
    expect(getComponentPresentation('NOR_GATE')).toEqual({ backend: 'raster', bareBody: true, markerless: true })
    const { container } = render(<NorGatePart />)
    ...
    expect(img.getAttribute('src')).toBe('/assets/components/nor-gate/nor-gate.default.1x.png')
    expect(img.getAttribute('srcset')).toContain('nor-gate.default.3x.png 3x')
    expect(container.querySelector('source').getAttribute('srcset')).toContain('nor-gate.default.3x.webp 3x')
  ...
  it('exterior background is genuinely transparent, not an opaque white rectangle', () => {
    ...
    for (const x of [489, 765, 1036]) expect(alphaAt(x, 890)).toBe(255)
  ...
  it('runtime inventory, dimensions, hashes and integrity are truthful', () => {
    ...
    expect(m).toMatchObject({ component: 'NOR_GATE', backend: 'raster', canonical: { width: 144, height: 96 } })
    expect(m.assets.map(a => a.file).sort()).toEqual(['nor-gate.default.1x.png', 'nor-gate.default.1x.webp', 'nor-gate.default.3x.png', 'nor-gate.default.3x.webp'])
...
describe('A9-NOR physical fit through existing generic contracts', () => {
  const bb = { id: 'bb', position: { x: 0, y: 0 }, layout: 'STANDARD_V1' }
  const origin = { x: 0, y: -47 }
  const def = getComponentDef('NOR_GATE')
  it('three exact contacts, pitch 12, short measured roots, no clipping', () => {
    ...
    const g = resolveAssemblyGeometry(createComponent('NOR_GATE', 0, 0), null)
    expect(g.contacts.map(c => [c.root.x, c.root.y])).toEqual([[45.890625, 83.4375], [71.765625, 83.4375], [97.171875, 83.4375]])
    expect(json('manifest.json').derivation.pixelProbe.map(p => p.runtimeRoot)).toEqual(g.contacts.map(c => [c.root.x, c.root.y]))
    expect(getAssemblyProfile('NOR_GATE').bodyClip).toBeUndefined()
    // Own measured max root-to-contact distance for NOR_GATE (~2.15px on A),
    // slightly above NAND's own measured 2px — real geometry, not copied.
    for (const c of g.contacts) expect(Math.hypot(c.root.x - c.target.x, c.root.y - c.target.y)).toBeLessThan(2.2)
  })
  it('all three contacts insert exactly onto distinct breadboard nets', () => {
    const placement = computeBreadboardPlacement(bb, 'NOR_GATE', origin, [])
    expect(placement).toMatchObject({ compatible: true, valid: true, breadboardActive: true, position: origin })
    const comp = createComponent('NOR_GATE', origin.x, origin.y)
  ...
  it.each([true, false])('real Document adapter connects Q through breadboard to another gate (A=%s)', high => {
    const doc = {
      ...
      components: [
        ...
        { id: 'g1', type: 'NOR_GATE', position: origin },
        // g2.A at (96,48), same column/net as g1.Q at (96,36).
        { id: 'g2', type: 'NOR_GATE', position: { x: 48, y: -35 } },
      ],
      wires: [
    ...
    // g1: A=high?HIGH:LOW, B=LOW => Q = NOR(A,LOW) = !A (since B decisive only when LOW+LOW)
    const g1Q = high ? Signal.LOW : Signal.HIGH
    // g2: A=g1.Q, B=LOW => Q = NOR(g1Q, LOW)
