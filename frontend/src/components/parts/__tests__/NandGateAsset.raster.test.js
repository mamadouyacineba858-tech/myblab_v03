...
// Asset-only lock for the A9-NAND raster pack. NAND_GATE has no functional
// component/registration/simulation contract yet (that is a separate,
// not-yet-authorized ticket) — this only guards the Founder reference and
// its derivatives against a regression to the opaque-white-rectangle defect
// that was found and fixed the same way on A9-OR.
...
const priorOpaqueSha = '676a9f047f9bbcb4a353cf612806530876b91e672371a3880e343620e4bdf96d'
...
describe('A9-NAND frozen raster asset (transparency lock, no applicative logic)', () => {
  ...
  it('exterior background is genuinely transparent, not an opaque white rectangle', () => {
    ...
    // uniformly transparent; both real states must be present in bulk.
  ...
  it('runtime derivatives carry real alpha at the exact canonical dimensions, and manifest/integrity are truthful', () => {
    ...
    expect(m.component).toBe('NAND_GATE')
    expect([m.canonical.width, m.canonical.height]).toEqual([144, 96])
    expect(m.assets.map(a => a.file).sort()).toEqual([
      'nand-gate.default.1x.png', 'nand-gate.default.1x.webp',
      'nand-gate.default.3x.png', 'nand-gate.default.3x.webp',
    ])
