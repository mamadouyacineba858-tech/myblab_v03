...
import { XorGatePart } from '../XorGatePart.jsx'
...
// reference, same method as OrGatePart.raster.test.jsx/NandGatePart.raster.test.jsx/NorGatePart.raster.test.jsx.
...
describe('A9-XOR raster and frozen source', () => {
  it('registered raster renderer uses only the approved picture', () => {
    expect(DEFAULT_REGISTRATIONS.filter(r => r.type === 'XOR_GATE')).toEqual([
      { type: 'XOR_GATE', component: XorGatePart, visual: { backend: 'raster' } },
    ])
    expect(getComponentPresentation('XOR_GATE')).toEqual({ backend: 'raster', bareBody: true, markerless: true })
    const { container } = render(<XorGatePart />)
    ...
    expect(img.getAttribute('src')).toBe('/assets/components/xor-gate/xor-gate.default.1x.png')
    expect(img.getAttribute('srcset')).toContain('xor-gate.default.3x.png 3x')
    expect(container.querySelector('source').getAttribute('srcset')).toContain('xor-gate.default.3x.webp 3x')
    expect(img.style.objectFit).toBe('contain')
  })
  it('Founder FROZEN bytes and actual dimensions match an independent lock (unchanged since A9-XOR-asset-transparency)', () => {
  ...
  it('exterior background is genuinely transparent, not an opaque white rectangle', () => {
    ...
    expect(opaque).toBeGreaterThan(450000)
    for (const x of [481, 759, 1033]) expect(alphaAt(x, 890)).toBe(255)
    expect(json('manifest.json').derivation.transparency).toMatchObject({
      priorSha256: '8ff6d6af57779a88019a385f6d0c480f9ecd558c9d41d6d4e06b84bf9a974fe9',
    })
  })
  it('runtime inventory, dimensions, hashes and integrity are truthful', () => {
    const m = json('manifest.json')
    expect(m).toMatchObject({ component: 'XOR_GATE', backend: 'raster', canonical: { width: 144, height: 96 } })
    expect(m.assets.map(a => a.file).sort()).toEqual(['xor-gate.default.1x.png', 'xor-gate.default.1x.webp', 'xor-gate.default.3x.png', 'xor-gate.default.3x.webp'])
...
describe('A9-XOR physical fit through existing generic contracts', () => {
  const bb = { id: 'bb', position: { x: 0, y: 0 }, layout: 'STANDARD_V1' }
  const origin = { x: 0, y: -47 }
  const def = getComponentDef('XOR_GATE')
  it('three exact contacts, pitch 12, short measured roots, no clipping', () => {
    ...
    const g = resolveAssemblyGeometry(createComponent('XOR_GATE', 0, 0), null)
    expect(g.contacts.map(c => [c.root.x, c.root.y])).toEqual([[45.09375, 83.4375], [71.15625, 83.4375], [96.890625, 83.4375]])
    expect(json('manifest.json').derivation.pixelProbe.map(p => p.runtimeRoot)).toEqual(g.contacts.map(c => [c.root.x, c.root.y]))
    expect(getAssemblyProfile('XOR_GATE').bodyClip).toBeUndefined()
    // Own measured max root-to-contact distance for XOR_GATE (~2.94px on A),
    // real geometry, not copied from AND/OR/NAND/NOR.
    for (const c of g.contacts) expect(Math.hypot(c.root.x - c.target.x, c.root.y - c.target.y)).toBeLessThan(3.0)
  })
  it('all three contacts insert exactly onto distinct breadboard nets', () => {
    const placement = computeBreadboardPlacement(bb, 'XOR_GATE', origin, [])
    expect(placement).toMatchObject({ compatible: true, valid: true, breadboardActive: true, position: origin })
    const comp = createComponent('XOR_GATE', origin.x, origin.y)
  ...
  it.each([true, false])('real Document adapter connects Q through breadboard to another gate (A=%s)', high => {
    const doc = {
      ...
      components: [
        ...
        { id: 'g1', type: 'XOR_GATE', position: origin },
        // g2.A at (96,48), same column/net as g1.Q at (96,36).
        { id: 'g2', type: 'XOR_GATE', position: { x: 48, y: -35 } },
      ],
      wires: [
    ...
    // g1: A=high?HIGH:LOW, B=HIGH => Q = XOR(A,HIGH) = !A (decisive B=HIGH inverts A).
    const g1Q = high ? Signal.LOW : Signal.HIGH
    // g2: A=g1.Q, B=HIGH => Q = XOR(g1Q, HIGH)
