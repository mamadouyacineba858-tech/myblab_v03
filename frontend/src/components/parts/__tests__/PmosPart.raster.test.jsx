...
import { PmosPart } from '../PmosPart.jsx'
...
const asset = (name) => resolve(dir, '../../../../public/assets/components/pmos', name)
const manifest = JSON.parse(readFileSync(asset('manifest.json'), 'utf8'))
const def = getComponentDef('PMOS')
describe('A8-PMOS electrical contract', () => {
  it('declares pins, schema, defaults, capabilities and executable model', () => {
    const entry = getCanonicalEntry('PMOS')
    ...
    expect(entry.parameterSchema[0].description).toMatch(/GATE LOW/)
    ...
    expect(getSimulationModel('PMOS').validate(entry.defaultParameters)).toBe(true)
  })
  for (const [drain, source] of [[Signal.HIGH, Signal.LOW], [Signal.LOW, Signal.HIGH]]) {
    it.each(Object.values(Signal))(`D/S ${drain}/${source}, GATE %s`, (gate) => {
      ...
      expect(getDcContribution('PMOS')({ pins, params, supplyVoltage: 12 })).toEqual({ voltage: 12, current: gate === Signal.LOW ? 0.6 : 0 })
  ...
  it.each([[Signal.HIGH, Signal.HIGH], [Signal.LOW, Signal.LOW], [Signal.UNKNOWN, Signal.LOW], [Signal.HIGH, Signal.FLOATING]])('incomplete D/S %s/%s returns null', (drain, source) => {
    expect(getDcContribution('PMOS')({ pins: { drain, source, gate: Signal.LOW }, params: { onResistance: 1 }, supplyVoltage: 5 })).toBeNull()
  })
  it('uses only the existing controlled-switch factory with LOW control', () => {
    const registry = readFileSync(resolve(dir, '../../../simulator/dcContributionRegistry.js'), 'utf8')
    expect(registry).toMatch(/const pmosDc = createControlledDcSwitchContribution\(\{\s*terminalAPinId: "drain",\s*terminalBPinId: "source",\s*controlPinId: "gate",\s*activeControlSignal: Signal.LOW,\s*\}\)/)
    for (const file of ['../../../canvas/CircuitComponent.jsx', '../../../utils/breadboardPlacementAdapter.js', '../../../utils/assemblyGeometry.js', '../../../utils/pinPresentationGeometry.js']) expect(readFileSync(resolve(dir, file), 'utf8')).not.toMatch(/PMOS/)
    expect(readFileSync(resolve(dir, '../PmosPart.jsx'), 'utf8')).not.toMatch(/simulator\//)
  })
  it('keeps PMOS absent from generic engines', () => {
    for (const file of ['resolution.js', 'engine.js', 'electricalAnalysis.js', 'scheduler.js', 'simulationRuntimeIntegration.js']) expect(readFileSync(resolve(dir, `../../../simulator/${file}`), 'utf8')).not.toMatch(/PMOS/)
  })
})
describe('A8-PMOS frozen raster', () => {
  it('registers renderer, raster presentation and palette', () => {
    expect(getComponentByType('PMOS')).toBe(PmosPart)
    expect(getComponentPresentation('PMOS')).toEqual({ backend: 'raster', bareBody: true, markerless: true })
    expect(PALETTE_ITEMS.find((p) => p.id === 'PMOS').label).toBe('MOSFET canal P')
  })
  it('uses WebP picture, PNG fallback and both density srcsets without SVG, distortion or data URLs', () => {
    const { container } = render(<PmosPart />)
    expect(container.querySelector('svg')).toBeNull()
    expect(readFileSync(resolve(dir, '../PmosPart.jsx'), 'utf8')).toMatch(/<img\s/)
    expect(container.querySelector('picture source').getAttribute('srcset')).toBe('/assets/components/pmos/pmos.default.1x.webp 1x, /assets/components/pmos/pmos.default.3x.webp 3x')
    const img = container.querySelector('img')
    expect(img.getAttribute('src')).toBe('/assets/components/pmos/pmos.default.1x.png')
  ...
  it('matches every frozen hash, byte length and PNG dimension', () => {
    ...
    expect(createHash('sha256').update(readFileSync(asset('REFERENCE.png'))).digest('hex'))
      .toBe('79805bcac6707365af0b8866e59877f18d853cc31ade79bc8e9086154542ef90')
  })
})
describe('A8-PMOS physical contacts and persistence', () => {
  it('joins three pins to three connectable and insertable contacts at exact pitch', () => {
    ...
    expect(contacts.map((c) => c.dx)).toEqual([60, 72, 84])
  ...
  it('bridges probed roots to actual breadboard holes and pin targets, clipping baked leads', () => {
    ...
    const component = { uid: 'pmos', type: 'PMOS', x: 36 - 60, y: 60 - 204 }
    const placement = computeBreadboardPlacement(bb, 'PMOS', { x: component.x + 1, y: component.y + 1 }, [])
    ...
    const roots = [[71, 171], [40, 171], [103, 171]]
    ...
    expect(def.height - getAssemblyProfile('PMOS').bodyClip.bottom).toBeCloseTo(171)
  })
  it('round-trips edited resistance and properties through Core and JSON', () => {
    const component = { ...createComponent('PMOS', 10, 20), parameters: { onResistance: 47 }, properties: { label: 'Q1' } }
    ...
    expect(result).toMatchObject({ type: 'PMOS', parameters: { onResistance: 47 }, properties: { label: 'Q1' } })
...
it('PMOS uses generic creation, Inspector resistance editing, export and import', () => {
  render(<CircuitProvider><InspectorProbe /></CircuitProvider>)
  act(() => circuitApi.addComponent('PMOS', 20, 40))
  ...
  expect(screen.getByText('MOSFET canal P')).toBeTruthy()
  ...
  expect(circuitApi.exportCircuit().components[0]).toMatchObject({ type: 'PMOS', parameters: { onResistance: 47 } })
})

it('clips the runtime at y171 through the existing CircuitComponent body convention', () => {
  const component = createComponent('PMOS', 20, 40)
  const { container } = render(<CircuitProvider><CircuitComponent component={component} /></CircuitProvider>)
  expect(container.querySelector('.circuit-component__body').style.clipPath).toBe('inset(0 0 117px 0)')
  expect(def.height - 117).toBe(171)
})
