...
describe('MB-L1-PROP-007 — DIODE physical lead consolidation', () => {
  it('keeps only the central raster body window and preserves the cathode-bearing asset', () => {
    ...
    expect(root.getAttribute('data-body-window')).toBe('24-60')
  ...
  it('declares two horizontal polished-metal leads without changing electrical endpoints', () => {
    const profile = getAssemblyProfile('DIODE')
    expect(profile).not.toBeNull()
    expect(profile.leads.anode).toEqual({ root: { dx: 24, dy: 15 }, style: 'metallic-wire' })
    expect(profile.leads.cathode).toEqual({ root: { dx: 60, dy: 15 }, style: 'metallic-wire' })
