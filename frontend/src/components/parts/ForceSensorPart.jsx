...
/**
 * FORCE_SENSOR — rendu raster A7-C2.
 *
 * Même mécanisme déclaratif que Tmp36Part.jsx / LightBulbPart.jsx /
 * VibrationMotorPart.jsx / HobbyGearmotorPart.jsx : paquet d'assets raster
 * réaliste Founder-approved (vue de face, asset 72×144 @1x / 216×432 @3x,
 * alpha réel, état unique `default`), backend `raster` déclaré dans
 * `defaultRegistrations.js`, `frontend/public/` servi à la racine web ->
 * `/assets/components/force-sensor/…`, priorité WebP via `<picture>`,
 ...
 */
const ASSET_DIR = '/assets/components/force-sensor'
const WEBP_SRCSET = `${ASSET_DIR}/force-sensor.default.1x.webp 1x, ${ASSET_DIR}/force-sensor.default.3x.webp 3x`
const PNG_SRCSET = `${ASSET_DIR}/force-sensor.default.1x.png 1x, ${ASSET_DIR}/force-sensor.default.3x.png 3x`
const PNG_FALLBACK = `${ASSET_DIR}/force-sensor.default.3x.png`

export function ForceSensorPart() {
  const def = getComponentDef('FORCE_SENSOR')
  const width = def?.width ?? 72
  const height = def?.height ?? 144

  return (
    <div className="part-force-sensor" aria-label="Capteur de force FSR">
      <picture className="part-force-sensor__picture">
        <source type="image/webp" srcSet={WEBP_SRCSET} />
        <img
          className="part-force-sensor__img"
          src={PNG_FALLBACK}
          srcSet={PNG_SRCSET}
          width={width}
          height={height}
